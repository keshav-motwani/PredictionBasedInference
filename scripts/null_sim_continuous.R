...
postpi_bs_modified = function(sim_dat_tv){
  ...
  for (i in 1:n_sim){
    ...
    bs_step = future_map(1:bs, .f = function(b){tryCatch({
      ...
      ## relationship model between continuous y and y_p
      ## we use gamma model for continuous outcomes gam(y ~ yp)
      rel_model = gam(y ~ s(pred), data = te_bs_data)
...
for (j in 1:length(beta1s)){
  ...
  file = paste0("results/null_postpi_sim_results_beta1_", beta1s[j], ".rds")
  ...
  if (beta1 == 0) {
    ...
    file = paste0("results/null_postpi_sim_results_beta1_", beta1s[j], "_qqplot.pdf")
...
file = paste0("results/null_postpi_sim_results", ".rds")
