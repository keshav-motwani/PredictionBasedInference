...
for (beta1 in c(0, 1)) {
  ...
  plot = ggplot(result, aes(x = n_val, y = bias / sqrt(reported_var), color = rep)) +
    ...
    geom_violin(data = assumed, aes(y = z), alpha = 1, fill = NA) +
    geom_hline(yintercept = -1.96, linetype = "dashed") + 
    geom_hline(yintercept = 1.96, linetype = "dashed") +
    labs(color = "Prediction Model", y = expression(frac(hat(beta)*' - '*beta^'*', hat('SE')*'('*hat(beta)*')')), x = expression(n[unlab])) +
    theme(legend.position = "bottom")
  ...
  file = paste0(result_path, "/main_postpi_sim_results_beta1_", beta1, "_distribution_plot.pdf")
