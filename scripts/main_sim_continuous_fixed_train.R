...
postpi_sim = function(ss, n_sim, beta1, beta2, beta3, beta4){
  ...
  print(head(sim_dat[sim_dat$set == "training" & sim_dat$sim == 1, ]))
  print(head(sim_dat[sim_dat$set == "training" & sim_dat$sim == 5, ]))
...
z_stat[["predpowinf"]] = df$predpowinf_beta / df$predpowinf_se
  ...
  file = paste0("results_observed_test_only_fixed_train/main_postpi_sim_results_beta1_", beta1s[j], "_ntraintest_", n_traintests[k], ".rds")
